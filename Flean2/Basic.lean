...
def NormalNumber.interp_pos {p : ℕ} (X : Type*)
    [Field X] [LinearOrder X] [IsStrictOrderedRing X] (f : NormalNumber p) :
    0 < f.interp X := by
  unfold interp
  have := f.bound.1
  refine mul_pos (Int.cast_pos.mpr ?_) ?_
  · apply lt_of_lt_of_le ?_ f.bound.1
    positivity
  positivity
